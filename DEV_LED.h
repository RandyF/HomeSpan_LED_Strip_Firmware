...
struct DEV_RgbLED : Service::LightBulb {       // RGB LED (Command Cathode)
  ...
  DEV_RgbLED(int red_pin, int green_pin, int blue_pin) : Service::LightBulb(){       // constructor() method
    ...
    V=new Characteristic::Brightness(20);     // instantiate the Brightness Characteristic with an initial value of 100%
  ...
  boolean update(){                         // update() method
    ...
    // AJR Changed to Low-Side
    redPin->set(100-R);                      // update the ledPin channels with new values
    greenPin->set(100-G);    
    bluePin->set(100-B);    
